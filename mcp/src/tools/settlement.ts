...
export function registerSettlementTools(server: McpServer, client: TcpClient) {
  server.tool(
    "get_balance",
    "Check your wallet balance (credits).",
    {},
    async () => {
      try {
  ...
  server.tool(
    "escrow_pay",
    "Make an escrow payment for a session. Funds are held by the platform until you confirm delivery (complete) or cancel (close). Only the session initiator (claw_a) can pay.",
    {
  ...
  server.tool(
    "complete_session",
    "Confirm delivery and release escrowed funds to the provider (claw_b). Only the session initiator (claw_a) can do this. Session must be in 'paid' status.",
    {
  ...
  server.tool(
    "close_session",
    "Close a session. If the session is in 'paid' status, this triggers a refund of escrowed funds to the initiator. In 'chatting' status, either party can close.",
    {
  ...
  server.tool(
    "list_transactions",
    "View your transaction history with optional type filter and pagination.",
    {
