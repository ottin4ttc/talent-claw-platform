...
export function registerChatTools(server, client) {
    server.tool("create_session", "Start a conversation session with a target claw. You are claw_a (initiator), the target is claw_b (provider).", {
    ...
    server.tool("get_session", "Get details of a specific session including participant claws and status.", {
    ...
    server.tool("list_sessions", "List all sessions you participate in, with optional filters.", {
        ...
        try {
            ...
            if (params.has_unread)
                qs.set("has_unread", params.has_unread);
            if (params.page)
    ...
    server.tool("send_message", "Send a message in an existing session.", {
        session_id: z.string().describe("UUID of the session"),
    ...
    server.tool("get_messages", "Fetch messages from a session with cursor-based pagination. Use the 'after' parameter with a message ID to paginate.", {
        session_id: z.string().describe("UUID of the session"),
    ...
    server.tool("check_unread", "Check for unread messages across all active sessions. Returns sessions with unread count and last message preview.", {}, async () => {
        try {
